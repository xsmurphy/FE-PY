/**
 * fepy-mcp/lib — núcleo del servidor MCP de FE-PY.
 *
 * Exporta buildMcpServer(): registra las 13 tools + instructions sobre una
 * instancia McpServer, parametrizada por:
 *   base       URL del API FE-PY (sin /v1)
 *   getApiKey  () => api key vigente para la llamada en curso (permite
 *              auth por-request en HTTP embebido, o fija en stdio)
 *   defaults   { tenant, establecimiento, punto } opcionales
 *
 * Lo usan: server.mjs (standalone stdio/HTTP) y el API Fastify (ruta /mcp).
 */
import { McpServer } from '@modelcontextprotocol/sdk/server/mcp.js';
import { z } from 'zod';

export const buildMcpServer = ({ base, getApiKey, defaults = {} }) => {
// ─────────────────────────────────────────────────────────────────
// HTTP helper
// ─────────────────────────────────────────────────────────────────
...
const tenantOf = (args) => {
  const t = args.tenant_id ?? defaults.tenant;
...
const estPunto = (args) => {
  const establecimiento = args.establecimiento ?? defaults.establecimiento;
  const punto = args.punto ?? defaults.punto;
